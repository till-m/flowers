...
from flowers.data.data_formatter import (
...
def param_norm(parameters):
    with torch.no_grad():
        total_norm = 0
        for p in parameters:
            total_norm += p.abs().pow(2).sum().item()
...
class Trainer:
    ...
    def save_model(self, epoch: int, validation_loss: float, output_path: str):
        """Save the model checkpoint. Only saves on rank 0 for distributed training."""
        if self.rank != 0:
            return
    ...
    def load_checkpoint(self, checkpoint_path: str):
        """Load the model checkpoint. Handles both DDP and non-DDP state dicts."""
        logger.info(f"Loading checkpoint from {checkpoint_path}")
        checkpoint = torch.load(checkpoint_path, weights_only=False, map_location=self.device)
        if self.model is not None:
            # Load into the unwrapped model (handles DDP wrapping)
        ...
        logger.info(f"Loading checkpoint from {checkpoint["epoch"]}")
        self.starting_epoch = (
            checkpoint["epoch"] + 1
    ...
    def normalize(self, batch):
        if hasattr(self, "dset_norm") and self.dset_norm:
            ...
            if "meta" in batch and batch["meta"] is not None:
                meta_scalar_names = self.datamodule.train_dataset.meta_scalar_names

                for i, scalar_name in enumerate(meta_scalar_names):
                    batch["meta"][:, i] = self.dset_norm.normalize(
                        batch["meta"][:, i], scalar_name
                    )
        return batch
    ...
    def denormalize(self, batch, prediction):
        if hasattr(self, "dset_norm") and self.dset_norm:
            ...
            if "meta" in batch and batch["meta"] is not None:
                ...
                for i, scalar_name in enumerate(meta_scalar_names):
                    batch["meta"][:, i] = self.dset_norm.denormalize(
                        batch["meta"][:, i], scalar_name
                    )
    ...
    def rollout_model(self, model, batch, formatter, train=True):
        ...
        moving_batch = batch
        if "input_fields" in moving_batch:
        ...
        if "meta" in moving_batch and moving_batch["meta"] is not None:
            moving_batch["meta"] = moving_batch["meta"].to(self.device)
        y_preds = []
        for i in range(rollout_steps):
            ...
            meta = inputs[1].to(self.device) if inputs[1] is not None else None
            y_pred = model(x, meta=meta)
